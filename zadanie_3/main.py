    ...
    def impulseChar(self, channelNum):
        if channelNum is None:
            messagebox.showerror(
                "Ошибка!",
                "Введите верное значение канала(целое число от 1 до 5)",
            )
            return ()
        ...
        for k in range(-M_2, M_2 + 1):
            if k == 0:
                ...
                h[k + M_2] = np.sin(np.pi * k * freqHigh / freqNeiqvist) / (
                    np.pi * k
                ) - np.sin(np.pi * k * freqLow / freqNeiqvist) / (np.pi * k)
                if k >= 0:
                    self.listBoxImpulse.insert(
                        tk.END,
                        f"{count}. \
                        {str(h[k + M_2])}",
                    )
    ...
    def channelCF(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Ошибка!",
                "Не был выбран канал "
                '(можно выбрать через кнопку "Характеристика h(i)")',
            )
            return ()
