...
M_2 = M // 2
freq_diskret = 2000  # частота дискретизации
freq_neiqvist = freq_diskret / 2
...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.impBtn = Button(
            self, text="Характеристика h(i)", command=partial(self.numChannel)
        )
    ...
    def graphImpEmpty(self):
        ...
        self.ax.set_title("Отсчеты импульсной хар-ки h(i). "
    ...
    def open_file(self):
        ...
        # построение графика
        global isx_canvas
        if isx_canvas is not None:
            isx_canvas.get_tk_widget().pack_forget()
        figIsx = Figure(figsize=(60, 2))
        self.ax = figIsx.add_subplot(111)
        self.ax.set_title("Исходный сигнал")
        self.ax.grid(True)
    ...
    def numChannel(self):
        ...
        channelNum = simpledialog.askinteger(
            "Ввод канала", "Введите номер канала (от 1 до 5):"
        )
        if (
            channelNum is not None
            and channelNum > 0
            and channelNum <= 5
            or not channelNum
        ):
    ...
    def impulseChar(self, channelNum):
        ...
        h[0] = (freqHigh - freqLow) / freq_neiqvist
        self.listBoxImpulse.insert(tk.END, "1) " + str(h[0]))

        for k in range(-M_2, M_2 + 1):
            if k == 0:
                h[k + M_2] = (freqHigh - freqLow) / freq_neiqvist
            else:
                h[k + M_2] = np.sin(np.pi * k * freqHigh / freq_neiqvist) / (
                    np.pi * k
                ) - np.sin(np.pi * k * freqLow / freq_neiqvist) / (np.pi * k)
                if k >= 0:
                    self.listBoxImpulse.insert(tk.END, f"{count}) "
                                               f"{str(h[k + M_2])}")
                    count += 1
    ...
    def channelCF(self):
        ...
        f0 = [
            ...
            160,
            200,
        ]  # Центральные частоты каналов (40, 80, 120, 160, 200 Гц)
        count = 1
        ...
        try:
            ...
            messagebox.showerror(
                "Ошибка!",
                'Не был выбран канал (можно выбрать через кнопку '
                '"Характеристика h(i)")',
            )
            return ()
        ...
        for n in range(M):
            if n == M_2:
                ...
                h[n] = (
                    np.sin(2 * cmath.pi * (n - M_2) * f2 / fs)
                    - np.sin(2 * cmath.pi * (n - M_2) * f1 / fs)
                ) / (cmath.pi * (n - M_2))
